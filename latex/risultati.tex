...
Si ponga ora la lunghezza media di un veicolo pari a $8$ m, la velocit\`a minima su una strada pari a $v_{min}=\frac{3}{4}v_{max}$ e una probabilit\`a di errore del $25$\% (DA RIVEDERE).
...
Si carichi quindi il sistema con 15000 veicoli, suddivisi in 1500 veicoli ogni 50 s, e si impedista a loro di uscire per 8000 s.


\subsection{Carico piccato}
Si inserisca ora nel sistema un totale di 27500 veicoli, suddivisi in 2750 veicoli ogni 50 s, impedendone la fuoriuscita dal reticolo.
...
\begin{figure}[H]
    ...
    mark=no,
    line width = 1.25pt
    ] plot file {./data/peaked_homo/120.dat};
    \end{axis}
    \end{tikzpicture}
...
\begin{figure}[H]
    ...
    mark size=0.69,
    draw=black,
    only marks] file {./temp_data/660_u-q.dat};
    \end{axis}
    \end{tikzpicture}\hfill
    \begin{tikzpicture}[scale=0.6]
        \begin{axis}[
        ...
        mark size=0.69,
        draw=black,
        only marks] file {./temp_data/360_q-k.dat};
        \end{axis}
    \end{tikzpicture}
    \centering
    \begin{tikzpicture}[scale=0.6]
    ...
    mark size=0.69,
    draw=black,
    only marks] file {./temp_data/720_u-k.dat};
    \end{axis}
    \end{tikzpicture}
