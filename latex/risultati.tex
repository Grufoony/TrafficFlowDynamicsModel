...
\begin{figure}[H]
    ...
    mark=no,
    line width = 1.25pt
    ] plot file {./temp_data/5300_t.dat};
    \addplot[
        domain=0:100,
        smooth,
        color=red,
...
Manca, ad esempio, la possibilit\`a di ricalcolare i \emph{best path} in base alle condizioni istantanee di traffico, oltre all'implementazione di alcune caratteristiche tipiche degli OVM (\emph{Optimal Velocity Model}) come la dipendenza della velocit\`a e della distanza di un veicolo in base a quella del veicolo che lo precede.
Ulteriori studi potrebbero essere svolti per verificare come queste revisioni impattino sui DFM, anche prese singolarmente.
